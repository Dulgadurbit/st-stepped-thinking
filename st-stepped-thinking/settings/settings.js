...
export async function loadSettings() {
    ...
    // Migrate existing thinking prompts to include model_override field
    migrateThinkingPromptModelOverride(settings.thinking_prompts);
    // Migrate existing thinking prompts to include group field
    migrateThinkingPromptGroup(settings.thinking_prompts);
    if (settings.character_settings) {
        for (const charSetting of settings.character_settings) {
            if (charSetting.thinking_prompts) {
                migrateThinkingPromptModelOverride(charSetting.thinking_prompts);
                migrateThinkingPromptGroup(charSetting.thinking_prompts);
            }
        }
    }
...
/**
 * Ensures all existing ThinkingPrompt objects have the model_override field.
 * @param {ThinkingPrompt[]} prompts
 * @return {void}
 */
function migrateThinkingPromptModelOverride(prompts) {
    if (!Array.isArray(prompts)) {
        return;
    }
    for (const prompt of prompts) {
        if (prompt.model_override === undefined) {
...
/**
 * Ensures all existing ThinkingPrompt objects have the group field.
 * @param {ThinkingPrompt[]} prompts
 * @return {void}
 */
function migrateThinkingPromptGroup(prompts) {
    if (!Array.isArray(prompts)) {
        return;
    }
    for (const prompt of prompts) {
        if (prompt.group === undefined) {
            prompt.group = '';
        }
    }
}
...
const defaultCommonSettings = {
    ...
    // per-step model and system prompt overrides
    'step_model_override': '',
    'final_reply_model_override': '',
    'step_system_prompt': '',
...
function loadCommonSettings() {
    ...
    // per-step model and system prompt overrides
    $('#stepthink_step_model_override').val(settings.step_model_override);
    $('#stepthink_final_reply_model_override').val(settings.final_reply_model_override);
    $('#stepthink_step_system_prompt').val(settings.step_system_prompt);
    $('#stepthink_final_reply_system_prompt').val(settings.final_reply_system_prompt);
}
...
function registerCommonSettingListeners() {
    ...
    // per-step model and system prompt overrides
    $('#stepthink_step_model_override').on('input', onTextareaInput('step_model_override'));
    $('#stepthink_final_reply_model_override').on('input', onTextareaInput('final_reply_model_override'));
    $('#stepthink_step_system_prompt').on('input', onTextareaInput('step_system_prompt'));
    $('#stepthink_final_reply_system_prompt').on('input', onTextareaInput('final_reply_system_prompt'));
 ...
 * @property {string} model_override - optional model override for this specific prompt step (empty = use global)
 * @property {string} group - the group this prompt belongs to (empty string = ungrouped)
 */
...
class ThinkingPromptSettings {
     ...
     * @return {number}
     */
    push(name, prompt, isEnabled, group = '') {
        const id = this.#getLowestFreeId();
        this.#settings.push({ id: id, name: name, prompt: prompt, is_enabled: isEnabled, model_override: '', group: group });
     ...
     * @return {void}
     */
    updateModelOverride(id, modelOverride) {
        const setting = this.getSettingBy(id);
        setting.model_override = modelOverride;
    }
    ...
    /**
     * Returns all unique group names (excluding empty string) in order of first appearance.
     * @return {string[]}
     */
    getGroupNames() {
        const seen = new Set();
        const groups = [];
        for (const prompt of this.#settings) {
            if (prompt.group && !seen.has(prompt.group)) {
                seen.add(prompt.group);
                groups.push(prompt.group);
            }
        }
        return groups;
    }
    ...
    /**
     * Sets is_enabled on all prompts belonging to a group.
     * @param {string} groupName
     * @param {boolean} isEnabled
     * @return {void}
     */
    setGroupEnabled(groupName, isEnabled) {
        for (const prompt of this.#settings) {
            if (prompt.group === groupName) {
                prompt.is_enabled = isEnabled;
            }
        }
    }

    /**
     * Checks if all prompts in a group are enabled.
     * Returns null if the group has no prompts.
     * @param {string} groupName
     * @return {?boolean}
     */
    isGroupEnabled(groupName) {
        const groupPrompts = this.#settings.filter(p => p.group === groupName);
        if (groupPrompts.length === 0) return null;
        return groupPrompts.every(p => p.is_enabled !== false);
    }

    /**
     * Returns the tri-state enabled status of a group.
     * @param {string} groupName
     * @return {'all'|'none'|'some'|'empty'}
     */
    getGroupEnabledState(groupName) {
        const groupPrompts = this.#settings.filter(p => p.group === groupName);
        if (groupPrompts.length === 0) return 'empty';
        const enabledCount = groupPrompts.filter(p => p.is_enabled !== false).length;
        if (enabledCount === groupPrompts.length) return 'all';
        if (enabledCount === 0) return 'none';
        return 'some';
    }

    /**
     * Removes all prompts belonging to a group.
     * @param {string} groupName
     * @return {void}
     */
    removeGroup(groupName) {
        for (let i = this.#settings.length - 1; i >= 0; i--) {
            if (this.#settings[i].group === groupName) {
    ...
    /**
     * Renames a group across all its prompts.
...
class ThinkingPromptList {
    ...
    #promptSettings;
    /**
     * Guard flag: when true, the group checkbox is being updated programmatically
     * and its change handler should NOT fire side effects.
     * @type {boolean}
     */
    #isUpdatingGroupCheckbox = false;
    ...
    static create(promptSettings, owner = null) {
        ...
        // Root-level sortable: handles group containers and ungrouped prompts
        rootElement.addClass('ui-sortable');
        rootElement.sortable({
            delay: getSortableDelay(),
            handle: '.drag-handle',
            tolerance: 'pointer',
            items: '> .stepthink_group_container, > [id^="stepthink_prompt_item"]',
            update: result.onRootReorder(),
            receive: result.onRootReceive(),
        });

        $('#stepthink_prompt_list_add' + suffix).on('click', result.onPromptItemAdd());
        $('#stepthink_prompt_group_add' + suffix).on('click', result.onGroupAdd());
    ...
    /**
     * Handles reordering at the root level (groups and ungrouped prompts).
     * Reconstructs the flat array order from the DOM.
     * @return {function(): void}
     */
    onRootReorder() {
        return () => {
    ...
    /**
     * Handles a prompt being dropped into the root (making it ungrouped).
     * @return {function(): void}
     */
    onRootReceive() {
        return (event, ui) => {
            ...
            if (idMatch) {
                ...
                this.#promptSettings.updateGroup(id, '');
            }
            this.#syncOrderFromDOM();
            saveSettingsDebounced();
        };
    ...
    /**
     * Handles reordering within a group's prompt list.
     * @return {function(): void}
     */
    onGroupPromptReorder() {
        return () => {
            this.#syncOrderFromDOM();
            saveSettingsDebounced();
        };
    ...
    /**
     * Creates a handler for when a prompt is dropped into a group.
     * @param {string} groupName
     * @return {function(): void}
     */
    onGroupPromptReceive(groupName) {
        return (event, ui) => {
            const item = ui.item[0];
            const idMatch = item.id?.match(/--(\d+)$/);
            if (idMatch) {
                const id = parseInt(idMatch[1]);
                this.#promptSettings.updateGroup(id, groupName);
            }
            this.#syncOrderFromDOM();
            saveSettingsDebounced();
        };
    ...
    onGroupAdd() {
        return () => {
            ...
            const existingGroups = this.#promptSettings.getGroupNames();
            ...
            this.#addSetting('New Prompt', '', true, groupName);
            saveSettingsDebounced();
        };
    }
    ...
    onPromptItemEnable() {
        return (event) => {
            ...
            const prompt = this.#promptSettings.getSettingBy(id);
            if (prompt && prompt.group) {
                this.#updateGroupCheckboxState(prompt.group);
            }
     ...
     * @return {(function(): void)}
     */
    onPromptItemModelOverride() {
        return (event) => {
            const id = Number(event.target.getAttribute('data-id'));
            const value = event.target.value;

            this.#promptSettings.updateModelOverride(id, value);
    ...
    onPromptItemRemove() {
        return (event) => {
            const id = Number(event.target.getAttribute('data-id'));
            const prompt = this.#promptSettings.getSettingBy(id);
            const groupName = prompt ? prompt.group : '';

            $(`#stepthink_prompt_item--${this.#promptSettings?.owner}--${id}`).remove();
            this.#promptSettings.remove(id);

            // If the group is now empty, remove the group container
            if (groupName) {
                const remainingInGroup = [...this.#promptSettings].filter(p => p.group === groupName);
                if (remainingInGroup.length === 0) {
                    $(`#stepthink_prompt_group--${this.#promptSettings?.owner}--${this.#cssEscape(groupName)}`).remove();
                }
            }
    ...
    renderAll() {
        ...
        // Group prompts by their group field, preserving order
        const groups = this.#buildGroupOrder();

        for (const groupName of groups) {
            if (groupName === '') {
                // Ungrouped prompts render directly into root
                for (const prompt of this.#promptSettings) {
                    if (prompt.group === '' || prompt.group === undefined) {
                        this.#renderPromptItem(prompt.id);
                    }
                }
            } else {
     ...
     * @return {void}
     */
    #addSetting(name = '', prompt = '', isEnabled = true, group = '') {
        const id = this.#promptSettings.push(name, prompt, isEnabled, group);

        if (group) {
            // Check if group container already exists
            const groupContainer = document.getElementById(
                `stepthink_prompt_group--${this.#promptSettings.owner}--${this.#cssEscape(group)}`
            );
            if (groupContainer) {
                // Append to existing group
                const promptList = groupContainer.querySelector(':scope > .stepthink_group_prompts');
                if (promptList) {
                    promptList.append(this.#createPromptItemElement(id));
                }
            } else {
            ...
            // Ungrouped — render directly into root
            this.#renderPromptItem(id);
        }
    }

    /**
     * Build an ordered list of group names from the prompts array.
     * Preserves first-appearance order. Always includes '' (ungrouped) if any ungrouped prompts exist.
     * @return {string[]}
     */
    #buildGroupOrder() {
        const seen = new Set();
        ...
        // Insert ungrouped at the beginning if any exist
        if (hasUngrouped) {
            groups.unshift('');
        }
    ...
    /**
     * Renders a full group section (header + all its prompts).
     * @param {string} groupName
     * @return {void}
     */
    #renderGroup(groupName) {
        const owner = this.#promptSettings.owner;
        const groupContainer = document.createElement('div');
        groupContainer.setAttribute('id', `stepthink_prompt_group--${owner}--${this.#cssEscape(groupName)}`);
        groupContainer.classList.add('stepthink_group_container');
        groupContainer.setAttribute('data-group', groupName);

        // Group header
        const header = this.#createGroupHeader(groupName);
        groupContainer.append(header);

        // Prompt list container (sortable within group)
        const promptList = document.createElement('div');
        promptList.classList.add('stepthink_group_prompts');
        promptList.setAttribute('data-group', groupName);

        // Render all prompts belonging to this group
        for (const prompt of this.#promptSettings) {
            if ((prompt.group || '') === groupName) {
                promptList.append(this.#createPromptItemElement(prompt.id));
            }
        }

        groupContainer.append(promptList);
        this.#rootElement.append(groupContainer);

        // Make the group's prompt list sortable
        $(promptList).addClass('ui-sortable');
        $(promptList).sortable({
            delay: getSortableDelay(),
            handle: '.drag-handle',
            tolerance: 'pointer',
            connectWith: `.stepthink_group_prompts[data-group], #stepthink_prompt_list${owner ? '--' + owner : ''}`,
            update: this.onGroupPromptReorder(),
            receive: this.onGroupPromptReceive(groupName),
        });
    }

    /**
     * Creates the group header with collapse toggle, name, enable checkbox, add prompt button, and delete button.
     * @param {string} groupName
    ...
    #createGroupHeader(groupName) {
        ...
        header.classList.add('stepthink_group_header', 'flex-container', 'alignItemsCenter', 'flexFlowRow');

        // Collapse toggle
        const toggle = document.createElement('span');
        toggle.classList.add('stepthink_group_toggle');
        toggle.innerHTML = '▶';
        toggle.addEventListener('click', () => {
            const container = header.closest('.stepthink_group_container');
            const prompts = container?.querySelector(':scope > .stepthink_group_prompts');
            if (prompts) {
                const isOpen = prompts.style.display !== 'none';
                prompts.style.display = isOpen ? 'none' : '';
                toggle.innerHTML = isOpen ? '▶' : '▼';
                container.classList.toggle('stepthink_group_collapsed', isOpen);
            }
        });
        ...
        nameInput.classList.add('text_pole', 'textarea_compact', 'stepthink_group_name_input');
        nameInput.value = groupName;
        nameInput.setAttribute('title', 'Group name — edit to rename the group');
        nameInput.addEventListener('change', (event) => {
            const newName = event.target.value.trim();
            // Read current group name from the container's data attribute
            const container = header.closest('.stepthink_group_container');
            const currentName = container?.getAttribute('data-group') || groupName;
            ...
            if (existingGroups.includes(newName)) {
                toastr.warning(`A group named "${newName}" already exists.`, 'Stepped Thinking');
                event.target.value = currentName;
                return;
            }
            this.#promptSettings.renameGroup(currentName, newName);

            // Update DOM IDs and data attributes
            if (container) {
                container.setAttribute('id', `stepthink_prompt_group--${owner}--${this.#cssEscape(newName)}`);
                container.setAttribute('data-group', newName);
                const promptListEl = container.querySelector(':scope > .stepthink_group_prompts');
                if (promptListEl) {
                    promptListEl.setAttribute('data-group', newName);
                }
                // Update the enable checkbox's data-group attribute
                const checkbox = container.querySelector(':scope > .stepthink_group_header .stepthink_group_enable');
                if (checkbox) {
                    checkbox.setAttribute('data-group', newName);
                }
            }
        ...
        enableCheckbox.setAttribute('title', 'Enable/disable all prompts in this group');
        enableCheckbox.classList.add('stepthink_group_enable');
        enableCheckbox.setAttribute('data-group', groupName);
        const groupEnabledState = this.#promptSettings.getGroupEnabledState(groupName);
        enableCheckbox.checked = groupEnabledState === 'all';
        enableCheckbox.indeterminate = groupEnabledState === 'some';
        enableCheckbox.addEventListener('change', (event) => {
            // Ignore programmatic updates — only respond to user clicks
            if (this.#isUpdatingGroupCheckbox) return;

            const isChecked = event.target.checked;
            // Clear indeterminate state — user explicitly clicked, so it's now fully checked or unchecked
            event.target.indeterminate = false;
            // Read current group name from the container's data attribute (in case it was renamed)
            const container = header.closest('.stepthink_group_container');
            const currentGroupName = container?.getAttribute('data-group') || groupName;
            this.#promptSettings.setGroupEnabled(currentGroupName, isChecked);

            // Update individual prompt checkboxes in this group
            if (container) {
                const checkboxes = container.querySelectorAll('.stepthink_prompt_enable');
        ...
        // Add prompt to group button
        const addPromptButton = document.createElement('div');
        addPromptButton.classList.add('menu_button', 'menu_button_icon', 'fa-solid', 'fa-plus', 'stepthink_group_add_prompt');
        addPromptButton.setAttribute('title', 'Add prompt to this group');
        addPromptButton.addEventListener('click', () => {
            // Read current group name from the container's data attribute (in case it was renamed)
            const container = header.closest('.stepthink_group_container');
            const currentGroupName = container?.getAttribute('data-group') || groupName;
            this.#addSetting('New Prompt', '', true, currentGroupName);
            saveSettingsDebounced();
        });

        // Delete group button
        const deleteButton = document.createElement('div');
        deleteButton.classList.add('menu_button', 'menu_button_icon', 'fa-solid', 'fa-trash', 'redWarningBG', 'stepthink_group_delete');
        deleteButton.setAttribute('title', 'Delete this group and all its prompts');
        deleteButton.addEventListener('click', async () => {
            // Read current group name from the container's data attribute (in case it was renamed)
            const container = header.closest('.stepthink_group_container');
            const currentGroupName = container?.getAttribute('data-group') || groupName;
            const confirmation = await callGenericPopup(
                `Delete the group "${currentGroupName}" and all its prompts?`,
                POPUP_TYPE.CONFIRM,
            );
            if (!confirmation) return;

            this.#promptSettings.removeGroup(currentGroupName);
            $(`#stepthink_prompt_group--${owner}--${this.#cssEscape(currentGroupName)}`).remove();
            saveSettingsDebounced();
        });

        header.append(toggle, handle, nameInput, enableCheckbox, addPromptButton, deleteButton);

        return header;
    }

    /**
     * Updates the group checkbox to reflect the current state of its prompts.
     * Uses tri-state: checked (all enabled), unchecked (none enabled), indeterminate (some enabled).
     * Sets a guard flag so the change handler doesn't fire during programmatic updates.
     * @param {string} groupName
     * @return {void}
     */
    #updateGroupCheckboxState(groupName) {
        const owner = this.#promptSettings.owner;
        const state = this.#promptSettings.getGroupEnabledState(groupName);
        const container = document.getElementById(`stepthink_prompt_group--${owner}--${this.#cssEscape(groupName)}`);
        if (container) {
            const checkbox = container.querySelector(':scope > .stepthink_group_header .stepthink_group_enable');
            if (checkbox) {
                this.#isUpdatingGroupCheckbox = true;
                checkbox.checked = state === 'all';
                checkbox.indeterminate = state === 'some';
                this.#isUpdatingGroupCheckbox = false;
            }
        }
    }

    /**
     * Renders a single ungrouped prompt item directly into the root element.
     * @param {number} id
     * @return {void}
     */
    #renderPromptItem(id) {
        this.#rootElement.append(this.#createPromptItemElement(id));
    }

    /**
     * Creates a prompt item DOM element.
     * @param {number} id
     * @return {HTMLDivElement}
     */
    #createPromptItemElement(id) {
    ...
    #renderPromptColumnContainer(id) {
        ...
        container.append(
            ...
            this.#renderModelOverrideRowContainer(id),
     ...
     * @return {HTMLDivElement}
     */
    #renderModelOverrideRowContainer(id) {
        const currentSetting = this.#promptSettings.getSettingBy(id);
        ...
        label.setAttribute('title', 'Override the AI model for this specific thinking step. Leave empty to use the global step model or the default SillyTavern model.');
        label.innerText = 'Model:';

        const modelInput = document.createElement('input');
        modelInput.setAttribute('data-id', String(id));
        modelInput.classList.add('text_pole', 'textarea_compact');
        modelInput.setAttribute('placeholder', 'e.g., gpt-4o-mini (leave empty for default)');
        modelInput.value = currentSetting.model_override || '';

        modelInput.addEventListener('input', this.onPromptItemModelOverride());

        container.append(label, modelInput);
    ...
    #renderButtonsColumnContainer(id) {
        ...
        isEnabledButton.classList.add('stepthink_prompt_enable');
    ...
    /**
     * Escape a string for use in a CSS ID selector.
     * @param {string} str
     * @return {string}
     */
    #cssEscape(str) {
    ...
    /**
     * Reconstructs the flat prompt array order from the current DOM state.
     * Walks the root element's children in order:
     * - Group containers: walks their .stepthink_group_prompts children
     * - Ungrouped prompt items: reads their ID directly
     * Then calls reorder() with the resulting ID list.
     * @return {void}
     */
    #syncOrderFromDOM() {
        const newOrderIds = [];
        const rootChildren = this.#rootElement[0]?.children || this.#rootElement.children;

        for (const child of rootChildren) {
            if (child.classList.contains('stepthink_group_container')) {
                // Walk prompts inside this group
                const promptList = child.querySelector(':scope > .stepthink_group_prompts');
                if (promptList) {
                    for (const promptItem of promptList.children) {
                        const idMatch = promptItem.id?.match(/--(\d+)$/);
                        if (idMatch) {
                            newOrderIds.push(parseInt(idMatch[1]));
                        }
                    }
                }
            } else if (child.id?.startsWith('stepthink_prompt_item')) {
                // Ungrouped prompt
                const idMatch = child.id.match(/--(\d+)$/);
                if (idMatch) {
                    newOrderIds.push(parseInt(idMatch[1]));
                }
            }
