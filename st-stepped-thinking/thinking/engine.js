...
import { generationCaptured, releaseGeneration, setFinalReplyGenerating, setThinkingStepGenerating } from '../interconnection.js';
...
import { swapModelTo, restoreModel, getStepModelOverride } from './model_override.js';
...
async function stopChatThinking() {
    setFinalReplyGenerating(false);
    setThinkingStepGenerating(false);
...
export async function runNewThoughtsGeneration(textarea, targetPromptIds = null) {
    ...
    try {
        ...
        // After thinking steps are done, the main generation will produce the final reply.
        // Set the flag so prompt_adjustment.js knows to apply the final reply system prompt.
        setFinalReplyGenerating(true);
    } finally {
        isThinking = false;
        releaseGeneration();
    }
...
export async function runNewBoundThoughtsGeneration(textarea, targetPromptIds) {
    ...
    // --- Final reply model override ---
    let cachedFinalModel = null;
    if (settings.final_reply_model_override && settings.final_reply_model_override.trim() !== '') {
        cachedFinalModel = swapModelTo(settings.final_reply_model_override.trim());
    }

    Generate('normal', { force_chid: characterId })
        .finally(() => {
            // --- Restore final reply model ---
            if (cachedFinalModel !== null) {
                restoreModel(cachedFinalModel);
            }
...
async function generateThoughts() {
    ...
    for (let i = 0; i < prompts.length; i++) {
        if (prompts[i].prompt) {
            const generatedThought = await generateCharacterThought(prompts[i].prompt, prompts[i]);
...
/**
 * @param {string} prompt
 * @param {?ThinkingPrompt} thinkingPrompt - the thinking prompt object for per-prompt model override
 * @return {Promise<string>}
 */
async function generateCharacterThought(prompt, thinkingPrompt = null) {
    const context = getContext();

    // --- Per-step model override ---
    const stepModel = getStepModelOverride(thinkingPrompt);
    let cachedStepModel = null;
    if (stepModel !== '') {
        cachedStepModel = swapModelTo(stepModel);
    }

    // Set flag so prompt_adjustment.js knows to apply the step system prompt
    setThinkingStepGenerating(true);
    ...
    try {
        ...
        // --- Restore model ---
        if (cachedStepModel !== null) {
            restoreModel(cachedStepModel);
        }
